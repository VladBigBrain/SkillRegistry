...
5. Use `vault-patcher` only when an approved plan or approved artifact already exists and the user explicitly asks to apply it.
6. Use `practice-lab-builder` only when a ready theory pack, map, or other learning artifact already exists.
7. Use `note-map-builder` only when a structured integration artifact already exists.
8. Use `notes-from-map` only when a ready markdown map already exists.
